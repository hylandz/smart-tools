...
from PyQt6.QtGui import QFont, QColor, QDragEnterEvent, QDropEvent, QIcon, QAction

from tc02.protocol.zr_protocol import parse_protocol_data
...
class ParseWorker(QThread):
    ...
    def run(self):
        ...
        for idx, raw_data in enumerate(self.data_list, 1):
            ...
            try:
                # 解析报文
                parse_result = parse_protocol_data(raw_data)
                if not parse_result[0]:
                    # 生成失败模块，发送信号
                    fail_module = self._generate_fail_module(idx, raw_data, parse_result[1])
                    self.result_module_signal.emit(fail_module)
                    continue

                json_str = ProtocolParser.to_formatted_json(parse_result[2])  # 转json形式

                # 生成成功/警告模块，发送信号
                success_module = self._generate_success_module(idx, raw_data, parse_result, json_str)
    ...
    def _generate_success_module(self, idx: int, raw_hex: str, parse_data: tuple, json_str: str) -> str:
        """生成单条成功/警告的HTML模块"""
        # 长数据截断
        if len(raw_hex) > 120:
        ...
        # # 状态标识
        status_icon = "✅" if parse_data[0] else "⚠️"
        status_text = "解析成功" if parse_data[0] else "解析警告"
        status_color = "#10b981" if parse_data[0] else "#f59e0b"
        ...
        basic_info = f"""
            ...
            <table style='width: 100%; border-collapse: separate; border-spacing: 0 8px; margin-bottom: 15px;'>
                ...
                <tr>
                    ...
                    <td style='color: #1e293b;'>{len(raw_hex)} 字符 / {parse_data[2]['包长度'] + 6} 字节</td>
                </tr>
                <!-- 帧头校验 -->
                <tr>
                    <td style='color: #64748b; font-weight: 500;'>帧头校验</td>
                    <td style='color: {"#10b981" if parse_data[0] else "#ef4444"};'>
                        {"✅ 通过" if parse_data[2]["包头"] else "❌ 失败"}
                        （{parse_data[2]['包头']} - 帧起始标识（0xDDDD））
                    </td>
                </tr>
                <tr>
                    <td style='color: #64748b; font-weight: 500;'>帧尾校验</td>
                    <td style='color: {"#10b981" if parse_data[0] else "#ef4444"};'>
                        {"✅ 通过" if parse_data[2]["包尾"] else "❌ 失败"}
                        （{parse_data[2]["包尾"]} - 帧结束标识（0xFFFF））
                    </td>
                </tr>
                <tr>
                    <td style='color: #64748b; font-weight: 500;'>校验和校验</td>
                    <td style='color: {
                    "#10b981" if parse_data[0] else
                    "#f59e0b" if parse_data[2]["校验码"] else
                    "#94a3b8"
                    };'>
                        {
                            f"✅ 通过 （实际：{parse_data[2]["校验码"]}，计算：{parse_data[2]["校验码"]}）" if parse_data[2]["校验码"] else
                            f"⚠️ 不匹配 （实际：{parse_data[2]["校验码"]}，计算：）"
                            if parse_data[2]["校验码"] is None else "ℹ️ 数据不足"
                        }
                    </td>
                </tr>
            </table>
        ...
        warning_msg = f"""
            <p style='color: {status_color}; margin-top: 10px; font-size: 13px;'>
                {status_icon} 提示：{parse_data[1]}
            </p>
        """ if parse_data[0] else ""

        # 卡片模块
        card_border_color = "#10b981" if parse_data[0] else "#f59e0b"
        return f"""
...
class ProtocolAnalyzerWindow(QMainWindow):
    ...
    def init_window(self):
        """初始化窗口基础属性"""
        self.setWindowTitle(self.TITLE_NAME + self.TOOL_VERSION)
